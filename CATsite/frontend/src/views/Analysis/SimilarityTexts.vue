          ...
          <div class="downloaded-files">
            ...
            <div class="file-list">
              <div v-for="(file, key) in files" v-bind:key="file.id" class="files">
                <div :class="setActive(key)" v-on:click="addActiveBtn(key)">
                  <a>{{ key + 1 + ") " + file.name }}</a>
                  <span id="removeFile" class="remove-file" v-on:click.stop="removeFile(key)">✘</span>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    addActiveBtn(key) {
      for (var i = 0; i < this.activeBtns.length; i++) {
        if (key == this.activeBtns[i]) {
          this.activeBtns.splice(i, 1);
          return;
        }
      }
      this.activeBtns.push(key);
    },
    ...
    async submitFile() {
      console.log(this.activeBtns)
      if (this.activeBtns != null) {
        this.params.data = await analysis.similarityTexts(
          this.files,
          this.activeBtns
        );
      } else {
    ...
    removeFile(key) {
      ...
      for (var i = 0; i < this.activeBtns.length; i++) {
        if (this.activeBtns[i] > key) {
          this.activeBtns[i] = this.activeBtns[i] - 1;
        }
      }
    }
  },
